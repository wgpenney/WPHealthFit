...
uint32_t analyzeAcceleration(uint32_t* currentType, Counter* counter, LowPassFilter* filter, bool isDriving, int32_t sensitivity, AccelData* acceleration, uint32_t size, bool isSleeping) {
    ...
    if(*currentType == 0 && !isSleeping) {
      *currentType = 1;
		...
		if (*currentType > 1) {	// Walking or Jogging
			...
			counter->totalSteps += steps;
      counter->currentSteps = steps;
		}
